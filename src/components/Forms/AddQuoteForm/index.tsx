...
const AddQuoteForm: FC = () => {
	...
	const updateQuote = async (data: AddQuoteData) => {
		...
		try {
			await axios.patch(`/quotes/myquote`, finalData).then((res) => {
	...
	return (
		<form onSubmit={onSubmit} className='form'>
			<div className='form-element textarea'>
				<textarea
					{...register('message', { required: 'Quote is required' })}
					name='message'
					className='form-control'
					value={defaultMessageValue}
					onChange={(e) => setDefaultMessageValue(e.target.value)}></textarea>
				{errors.message && (
					<span className='form-text required'>{errors.message.message}</span>
				)}
			</div>
			<div className='buttons'>
				<input
					className='site-btn btn-orange'
					name='message'
					type='submit'
					value='Submit'
