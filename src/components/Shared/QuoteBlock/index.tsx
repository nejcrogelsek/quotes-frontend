...
const QuoteBlock: FC<Props> = ({ id, votes, message, user }: Props) => {
			...
			if (userValue) {
				axios
					.delete(`/votes/user/${user.id}/downvote`, {
						data: { quote_id: id, user_id: userValue.id },
					})
					.then((res) => {
						getVotes()
						setAllow(true)
	...
	useEffect(() => {
		if (votesValue && userValue) {
			for (let i: number = 0; i < votesValue.length; i++) {
				if (
					userValue.id === votesValue[i].user_id &&
					id === votesValue[i].quote_id
				) {
					setAllow(false)
				}
			}
	...
	const removeQuote = async () => {
		try {
			await axios
				.patch('/quotes/myquote', { message: '', user: userValue })
				.then((res) => {
			...
			{message === '' ? (
				...
				<>
					<div className='voting'>
						{userValue ? (
							<button
								className={allow ? 'quote-upvote' : 'quote-upvote not-allowed'}
								onClick={upvote}>
								<svg
							...
							<Link to='login' className='quote-upvote'>
								<svg
									xmlns='http://www.w3.org/2000/svg'
									width='16'
									height='16'
						...
						<span className='quote-votes'>{stateVotes}</span>
						{userValue ? (
